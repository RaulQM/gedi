...
class GeDi:
    ...
    def compute(self, pts, pcd):
        ...
        for b in range(int(np.ceil(len(pts) / self.samples_per_batch))):
            ...
            for i in range(i_start, i_end):
                ...
                _inds = out[i] #out[0][out[1][i]:out[1][i + 1]]
                try:
    ...
    def radius_search_pytorch_optimized(self, pcd, pts, radius):
        """
        Perform an optimized radius search using pairwise distance and find points within the given radius.
        """
        # Efficiently compute pairwise distances on the GPU if available
        dist_matrix = torch.cdist(pts, pcd, p=2)  # Euclidean distance (L2 norm)

        # Apply radius condition to get a binary mask (M x N)
        within_radius = dist_matrix <= radius.view(-1, 1)

        # Extract indices of points within the radius for each query point
        indices = within_radius.nonzero(as_tuple=True)[1].view(within_radius.shape[0], -1)
        
        return indices

    def compute_optimized(self, pts, pcd):
        """
        Main computation method to calculate point cloud descriptors.
        """
        radii = self.r_lrf * torch.ones(len(pts), device=pts.device)

        # Use the optimized radius search function
        out = self.radius_search_pytorch_optimized(pcd, pts, radii)
        ...
        for b in range(int(np.ceil(len(pts) / self.samples_per_batch))):
            ...
            # Pre-allocate space for point patches
            x = torch.empty(i_end - i_start, 3, self.samples_per_patch_lrf, device=pts.device)

            for j, i in enumerate(range(i_start, i_end)):
                _inds = out[i]

                # Efficient random sampling directly on the GPU
                if len(_inds) >= self.samples_per_patch_lrf:
                    inds = _inds[torch.randperm(len(_inds))[:self.samples_per_patch_lrf]]
                else:
                    # If there are fewer points, pad with random selections
                    extra_inds = torch.randint(0, len(_inds), (self.samples_per_patch_lrf - len(_inds),), device=pts.device)
                    inds = torch.cat([_inds, _inds[extra_inds]])

                # Assign selected points to the patch
                x[j] = pcd[inds].T

            # Feed the patches into the local reference frame (LRF) function
            patch = self.lrf(pts[i_start:i_end], x)

            with torch.no_grad():
                # f = self.gedi_net(patch.cuda())
                f = self.traced_model(patch)

            # Store the output in the descriptor array
            pcd_desc[i_start:i_end] = f.cpu().detach().numpy()[:i_end - i_start]
